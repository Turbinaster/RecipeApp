...
@Composable
fun VoiceRecipeScreen(navController: NavController, response: String?) {
    ...
    LaunchedEffect(response) {
        if (!response.isNullOrEmpty()) {
            try {
                ...
                Log.d("VoiceRecipeScreen", "Full response: $response")

                // Парсим как Map с transcription и recipe
                ...
                Log.d("VoiceRecipeScreen", "Recipe JSON: $recipeJson")

                if (!recipeJson.isNullOrEmpty()) {
                    // Очистка от возможных маркеров ```json
                    if (recipeJson.startsWith("```json\n") && recipeJson.endsWith("\n```")) {
                        recipeJson = recipeJson.substring(8, recipeJson.length - 4).trim()
                        Log.d("VoiceRecipeScreen", "Cleaned recipe JSON: $recipeJson")
                    }
                    recipeInfo = Json.decodeFromString<RecipeData>(recipeJson)
                    Log.d("VoiceRecipeScreen", "Recipe data parsed: $recipeInfo")
                } else {
                    Log.e("VoiceRecipeScreen", "Recipe JSON is empty")
                }
            } catch (e: Exception) {
                Log.e("VoiceRecipeScreen", "Failed to parse response: ${e.message}")
            }
        }
    }
    ...
    Box(
        ...
        Column(
            ...
            Column(
                ...
                // Список ингредиентов
                Text(
                    text = recipeInfo?.ingredients?.replace("\\n", "\n")?.takeUnless { it.isNullOrBlank() } ?: "Ингредиенты не получены",
                    modifier = Modifier.fillMaxWidth()
                )
