...
@Composable
fun VoiceRecipeScreen(navController: NavController, response: String?, inputType: String = "voice") {
    ...
    LaunchedEffect(response) {
        if (!response.isNullOrEmpty()) {
            try {
                ...
                transcription = jsonResponse["transcription"]?.takeIf { it.isNotBlank() }
                ...
                if (!recipeJson.isNullOrEmpty()) {
                    if (recipeJson.startsWith("```json\n") && recipeJson.endsWith("\n```")) {
                        ...
                        Log.d("VoiceRecipeScreen", "Cleaned JSON: $recipeJson")
                    }
                    recipeInfo = Json.decodeFromString<RecipeData>(recipeJson)
                    Log.d("VoiceRecipeScreen", "Parsed recipe: $recipeInfo")
                } else {
        ...
        Column(
            ...
            if (transcription != null) {
                Column(
                    ...
                    Text(
                        text = if (inputType == "voice") "Ваш запрос:" else "Ваш текстовый запрос:",
